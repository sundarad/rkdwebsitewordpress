...
const portfolioHubIsLodash = () => {
	let portfolioHubIsLodash = false;
	...
	if ( 'undefined' != typeof( _ ) && 'function' == typeof( _.forEach ) ) {
		...
		portfolioHubIsLodash  = true;

		funcs.forEach( function ( func ) {
			// If just one of the functions do not exist, then not lodash
			portfolioHubIsLodash = ( 'function' != typeof( _[ func ] ) ) ? false : portfolioHubIsLodash;
		} );
	}

	if ( portfolioHubIsLodash ) {
...
jQuery(document).ready(function () {
	...
	var portfolioHubLodashCounter = 0
	var portfolioHubLodashInterval = setInterval(function(){
		if ( portfolioHubIsLodash() ) {
			_.noConflict();
			clearInterval(portfolioHubLodashInterval);
		} else if(portfolioHubLodashCounter > 20) {
			clearInterval(portfolioHubLodashInterval);
		}
		portfolioHubLodashCounter++;
	}, 1000);
